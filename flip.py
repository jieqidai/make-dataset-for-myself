...
def main():
    imgs_dir ='F:/recognize/val/images/'
    xmls_dir = 'F:/recognize/val/xmls/'
    
    imgs_save_dir= 'D:/val/flip_imgs/'
    if not os.path.exists(imgs_save_dir):
        os.makedirs(imgs_save_dir)
    xmls_save_dir='D:/val/flip_xmls/'
    if not os.path.exists(xmls_save_dir):
        os.makedirs(xmls_save_dir)
    img_suffix=['.jpg','.png','.bmp']
    name_suffix='flip' #命名标识
    flip_types = [1] #指定每张图翻转类型 1水平翻转，0垂直翻转，-1水平垂直翻转
