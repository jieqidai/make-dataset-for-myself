...
def main():
    imgs_dir ='D:/pickup/origin/'
    bk_imgs_dir ='D:/pickup/back/'
    xmls_dir = 'D:/pickup/annotation/'
    
    rot_imgs_save_dir= 'D:/pickup/rot_imgs1/'
    if not os.path.exists(rot_imgs_save_dir):
        os.makedirs(rot_imgs_save_dir)
    rot_xmls_save_dir='D:/pickup/rot_xmls1/'
    if not os.path.exists(rot_xmls_save_dir):
        os.makedirs(rot_xmls_save_dir)
    img_suffix=['.jpg','.png','.bmp']
    name_suffix='rot' #命名标识
    rot_angles = [1.0,2.0] #指定旋转角度，当randomRotation=False时有效
    random_num=2 #随机旋转角度个数
    randomRotation=False #使用随机旋转
